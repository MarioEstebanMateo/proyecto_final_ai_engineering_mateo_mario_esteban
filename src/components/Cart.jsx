...
const Cart = ({ onNavigate }) => {
  ...
  const handleConfirmOrder = () => {
    ...
    Swal.fire({
      ...
      html: `
        <div class="text-left">
          <p class="mb-2"><strong>Cliente:</strong> ${customerInfo.nombre} ${customerInfo.apellido}</p>
          <p class="mb-2"><strong>Teléfono:</strong> ${customerInfo.telefono}</p>
          <p class="mb-2"><strong>Tipo:</strong> ${customerInfo.tipoEntrega === 'retiro' ? 'Retiro en local' : 'Delivery'}</p>
          ${customerInfo.tipoEntrega === 'delivery' ? `<p class="mb-2"><strong>Dirección:</strong> ${customerInfo.direccion}</p>` : ''}
          <p class="mb-2"><strong>Horario:</strong> ${customerInfo.horario}</p>
          <p class="mt-4 text-xl"><strong>Total: $${total.toFixed(2)}</strong></p>
        </div>
      `,
      icon: 'question',
      showCancelButton: true,
      ...
      <div className="card">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
            <p><strong>Cliente:</strong> {customerInfo.nombre} {customerInfo.apellido}</p>
            <p><strong>Teléfono:</strong> {customerInfo.telefono}</p>
            <p>
              <strong>Tipo:</strong> {customerInfo.tipoEntrega === 'retiro' ? 'Retiro en local' : 'Delivery'}
            </p>
            <p><strong>Horario:</strong> {customerInfo.horario}</p>
            {customerInfo.tipoEntrega === 'delivery' && (
              <p className="md:col-span-2"><strong>Dirección:</strong> {customerInfo.direccion}</p>
            )}
          </div>
          <button
        ...
        <div className="border-t-2 border-gray-200 dark:border-gray-600 pt-6">
          ...
          <button onClick={handleConfirmOrder} className="btn-primary w-full text-lg">
